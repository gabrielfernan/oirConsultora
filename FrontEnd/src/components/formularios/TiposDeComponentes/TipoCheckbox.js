...
const TipoCheckbox = ({
  ...
  const handleOpciones = (e, i) => {
    if (item.value) {
      let value = [...item.value];
      if (e.target.checked) {
        value.push(e.target.name);
      } else {
        value = value.filter(elem => elem !== e.target.name);
      }
      actualizarDatoComponente({index: index, prop: 'value', valor: value});
    } else {
      let value = [];
      if (e.target.checked) {
        value.push(e.target.name);
                ...
                >
                  ...
                  <FormGroup>
                    {item.options.map((opcion, i) => (
                      <FormControlLabel
                        control={<Checkbox name={opcion.etiqueta} />}
                        label={opcion.etiqueta}
                        key={i}
                        onChange={e => handleOpciones(e, i)}
                        disabled={edicion}
                      />
                    ))}
                  </FormGroup>
